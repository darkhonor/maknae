...
set -euo pipefail
case "${1:-$(uname -s)}" in
  Linux) absent='macos_mutation_directory_flags|macos_fd_path|unsupported_fd_path|portable_probe_openat2|macos_path_delegation_flags|macos_reopen_writable';;
  Darwin) absent='linux_mutation_directory_flags|linux_fd_path|linux_probe_openat2|openat2_resolve|unsupported_fd_path|linux_path_delegation_flags|linux_reopen_writable';;
