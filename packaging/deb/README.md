...
A fresh install is **not runnable**. `postinst` `enable`s the unit but never
`start`s it: the shipped `authz.yaml` uses `~` patterns and the DAC gate
fail-closes until `maknae enroll` writes the `principal`. The order is
load-bearing:
