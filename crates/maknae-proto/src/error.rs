...
#[derive(Debug)]
pub enum ProtoCodecError { Encode(String), Decode(String), UnsupportedVersion(u16) }
...
#[derive(Debug)]
pub enum ProtoFrameError { Oversize { declared: usize, max: usize }, Truncated, Io(String) }
impl std::fmt::Display for ProtoFrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ProtoFrameError::Oversize { declared, max } => write!(f, "frame oversize: declared {declared} > max {max}"),
