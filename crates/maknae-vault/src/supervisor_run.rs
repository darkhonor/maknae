...
use crate::supervisor::{retry_action, rotate_now, RetryAction};
...
async fn supervisor_loop(ctx: SupervisorCtx) -> VaultError {
    loop {
        ...
        let lease = ctx.current_lease_secs();
        if lease == 0 {
        ...
        // Independent leaf-rotation check, once per renewal cadence — decoupled
        // from the renewal outcome below.
        if let Err(e) = maybe_rotate_leaf(&ctx).await {
            return e;
        }

        // The token's ACTUAL remaining validity at this moment is ~1/3 of the lease
        // (lease - wait); track it as a real deadline (not a static estimate) so
        // backoff sleeps during retry correctly shrink the budget `retry_action`
        // reasons about.
        let deadline = Instant::now() + Duration::from_secs(lease.saturating_sub(wait));
        let retry_started = Instant::now();
        let mut attempt: u32 = 0;
        loop {
            match ctx.renew_token_once().await {
                Ok(_) => break, // back to the outer loop's normal cadence
                Err(_) => {
                    attempt += 1;
                    ...
                    match retry_action(remaining, elapsed, attempt) {
                        ...
                        RetryAction::FailClosed => return ctx.expire_now(),
                    }
                }
            }
        }
