...
4. **One persona per instance.** Multi-persona hosting is deliberately deferred behind a feature gate (a subscription-tier candidate) — one persona per instance is the operationally proven deployment pattern, and deny-by-default already answers the MVP's cross-persona question (no rule = no access). Memory `subject` scoping ships in the label schema from birth (KLC section 6), so later enablement is additive — a gate flip plus grant rules, never a migration.
