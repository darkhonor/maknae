...
`write` consumes raw stdin bytes within the configured frame limit. The kernel decides the write and records its intent; the CLI then replaces existing content, or creates an absent file exclusively, under your own credentials, and reports the outcome. A collision never silently overwrites another file. `read` prints a file's raw bytes: the kernel decides the read and records its intent, and the CLI reads the file under your own credentials, reports the byte count, and prints the bytes once that report is recorded. Delete without `--recursive` removes a file, symlink, or empty directory. Recursive delete requires authority over the entire subtree and never follows symlinks.
