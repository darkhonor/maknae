...
`maknaed` and `maknae` release binaries contain `aws-lc-fips-sys`, `aws-lc-rs`, `aws-lc-sys`, and `ring` in their embedded `cargo auditable` manifests. *(**Corrected 2026-09-19 (#320; [ADR-0025](../adr/ADR-0025-fips-validation-is-a-goal-not-a-constraint.md)) — `ring` no longer appears in any of this.** Measured on both `main` and the #320 branch: `cargo tree -i ring --target all` prints no tree, and the regenerated `THIRD-PARTY-NOTICES.md` drops the row. It left when `reqwest` moved 0.12 → 0.13 under `rustls-no-provider` — not through anything #320 did. **Three sections of this record rest on the old measurement and each is corrected in place: this one (§1), §3 in full, and §6.** The findings themselves are unaffected or strengthened: no Maknae call site ever selected ring, and one fewer present-but-dead crypto crate reaches the artifacts.)* `maknae-spifc` does not contain those crypto crates. `ring` is therefore compiled into the two Vault/TLS-bearing release binaries, but source review found no Maknae call site that explicitly selects `rustls::crypto::ring` or passes a ring provider. Maknae's own explicit provider builders use `rustls::crypto::aws_lc_rs::default_provider()`.
...
> **CORRECTED 2026-09-19 (#320; [ADR-0025](../adr/ADR-0025-fips-validation-is-a-goal-not-a-constraint.md)) — THIS SECTION'S PREMISE NO LONGER HOLDS: `ring` is not in the graph at all.** Everything below describes a tree in which `ring` entered through the `reqwest` / `hyper-rustls` / `rustls-webpki` stack and was *"compiled into the release artifacts for `maknaed` and `maknae`"*. **Measured 2026-09-19 on both `main` and the #320 branch: `cargo tree -i ring --target all` prints no tree** (it warns *"nothing to print"* and exits 0), and `ring` survives in `Cargo.lock` only as `rustls-webpki`'s unselected optional edge. It left when `reqwest` moved 0.12 → 0.13 under `rustls-no-provider` — **not** through anything #320 did; #320 merely measured it while correcting neighbouring claims, and this PR's regenerated `THIRD-PARTY-NOTICES.md` drops the `ring 0.17.14` row that `main`'s still carries. **The section's conclusion is unaffected and, if anything, strengthened**: no Maknae call site ever selected ring, and one fewer present-but-dead crypto crate reaches the artifacts. The `deny.toml` note at lines 13-23 and `crates/maknae-vault/src/fips.rs` carry the same measurement.
...
`reqwest 0.12.28` *(stale 2026-09-19: `main` resolves 0.13.5, and that 0.12 → 0.13 move is what removed ring)* will use a process-default rustls provider if one has been installed; if none is installed and its ring feature is compiled, it falls back to `rustls::crypto::ring::default_provider()`. Maknae's load-bearing control is therefore enforced at Vault-client construction, not only by caller convention: `crates/maknae-vault/src/client.rs` documents the load-bearing ordering and calls `assert_fips_provider()` inside `VaultClient::from_document_with_secret()` before building the `reqwest` Vault client. The daemon and CLI paths also install and assert the aws-lc-rs FIPS default before constructing Vault clients.

The `deny.toml` comment at lines 9-12 summarizes this as a present-but-dead `ring` dependency. *(Corrected 2026-09-19, #320: that line range is now the superseded half — the dated correction is at `deny.toml` lines 13-23, and it records that ring is not present at all.)* This record is the compiled-artifact evidence for that statement: `ring` is present in the shipped `maknaed` and `maknae` binaries, but is not selected by Maknae call sites under the current provider gate.
...
**ADDENDUM 2026-09-19 (#320; decision of record [ADR-0025](../adr/ADR-0025-fips-validation-is-a-goal-not-a-constraint.md)) — the module LINE has changed, and point 1 above needs one word struck.** Maknae has moved from the AWS-LC-FIPS **3.x** module to **4.x** (`aws-lc-rs` 1.18.1, `aws-lc-fips-sys` 0.14.2) so that RUSTSEC-2026-0285 could be taken; rustls 0.23.45 carries the fix and requires `aws-lc-rs >=1.18`. Upstream lists v4.0 static and v4.0 dynamic under *Modules In Process* ("Review"), not under validated (`aws-lc/crypto/fipsmodule/FIPS.md`, read 2026-09-19), so **point 1 now reads: the module differs by platform — static on Linux, dynamic on macOS — and NEITHER is on the CMVP validated list today.** Certificates #5314 and #5298 describe the line Maknae shipped until 2026-09-19 and are no longer an SC-13 citation for the built artifact. Maintainer ruling: FIPS 140-3 validation is Maknae's design goal, not a constraint in force. **This record's METHOD and its OS-crypto-independence findings stand unchanged** — they never turned on certificate status. *(Narrowed 2026-09-19, in the same change: an earlier draft of this addendum said "everything else in this record stands unchanged", which was wrong in three sections — §1, §3 and §6 all rest on a `ring` presence measured away, and each is corrected in place. Asserting completeness over a set I had not walked is exactly the failure this record's own subject warns about.)* Specifically: the graph is still registry-only, the Linux artifact still links no OS OpenSSL, and §5's native-code advisory-coverage gap (#191) is, if anything, sharpened, since the vendored AWS-LC snapshot it names has moved to a new major line. The dylib basename on macOS moves with the crate version: `libaws_lc_fips_0_14_2_crypto.dylib`.

Conclusion:

For the local Linux release build examined here, Maknae's shipped cryptography does not depend on OS-provided OpenSSL, NSS, GnuTLS, native-tls, or kernel crypto APIs. The crypto that reaches `maknaed` and `maknae` is vendored through registry crates, primarily AWS-LC and ring as described above. *(Corrected 2026-09-19, #320: **and no longer ring** — see the correction at the head of §3. AWS-LC alone now.)*
...
For `maknaed` and `maknae`, the embedded manifest lists `aws-lc-fips-sys`, `aws-lc-rs`, `aws-lc-sys`, and `ring`. *(Corrected 2026-09-19, #320: **no longer `ring`** — see the correction in §1. The visibility point this section makes is unchanged; the crate list it illustrates with is one shorter.)* That is valuable: an operator can see that the AWS-LC wrapper crates and ring are present in the binary.
