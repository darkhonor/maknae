...
**Mutation testing is risk-based and exists for external provability** (operator, 2026-08-04): MLS/DCS/security-critical crates (`mutants_crates`, initially the DCS classification engine — since relocated to a separate private library) must be provable to external third parties as fail-closed and policy-exact. Coverage says the code ran; killed mutants say the tests would notice if it lied.
...
- The coverage step adds a second full instrumented build+test run to CI plus tool installs (~minutes); the fixture suite's live-root modes add three small instrumented builds; the mutation job pulls its own toolchain when triggered. Accepted: the gate is the SA-11 evidence. A binstall/tool-cache optimization, the isolation-contract negative-control gap, and line-level diff coverage are **named follow-ons**. *(Corrected 2026-09-04, [#219](https://github.com/darkhonor/maknae/issues/219): the isolation-contract negative-control gap is CLOSED. `isolation-contract-lint.sh` was the only gate in `ci/gates/` with no probe at all — it resolved its root from the CWD and so could not be pointed at a fixture. It now takes the same `${1:-…}` root override its siblings have and carries a probe for every branch it has, including a floored accept against the real contract. The other two follow-ons remain open.)*
...
Refresh rule: re-measure on the CI lane and update the toml provenance when coverage materially changes; every column carries value/date/lane/command provenance.

**Adoption baseline (relocated):** the gate was first adopted (PR #37, 2026-08-04) against the DCS classification engine crate, which carried the only T1 production code at the time — a cohort ratchet of **floor 97** and a **zero-missed** mutation result at adoption. That crate has since moved to a separate private library; its concrete per-file baseline lives with it. The gate mechanism, the production-region metric, the cohort-ratchet discipline, and the fail-closed provability rationale are unchanged and stand for this repo; the T1 cohort and `mutants_crates` are empty until in-tree security-critical code returns, at which point a fresh baseline is measured and recorded (see `coverage-tiers.toml` provenance).
...
| Test-the-tests | Risk-based mutation, zero missed, dependency-selected on PR and main (2026-09-06 amendment) | SA-11(5) analog | mutation job logs |
...
Source spec (memory store); issue #24; `ci/gates/coverage-tiers.sh`, `ci/gates/coverage_check.py`, `ci/gates/tests/coverage-tiers/`; `coverage-tiers.toml`; ADR-0001 (+ its append-only amendment), ADR-0002 amendment; MPE-ES tiered-testing standard + Microkosmos runner-readiness (provenance only).


## Amendment — 2026-09-06, affected CI (#126)

The earlier unconditional-main mutation rule and path-only PR gate are superseded. They spent scarce hosted minutes on documentation-only merges while missing reverse-dependency and shared-input effects on PRs. `ci/affected.py` now compares a PR merge base to its head, and a push's before/after endpoints, with NUL-delimited paths and renames represented as deletion plus addition. It selects complete changed packages and their transitive reverse dependents across normal, development, build and target-specific dependencies; the mutation and Darwin cohorts intersect that closure. Missing history, empty diffs, shared configuration, unknown paths and uncertain dependency resolution select all. Invalid package or mutation authority fails. Documentation skips require an explicit allowlist, on both PR and main pushes. Required job names and unique main-run concurrency remain unchanged; a selector failure cannot silently skip those required controls.

Every source-changing run still measures the complete aggregate coverage report and enforces all file floors, partition rules and cohort ratchets. Partial coverage reports would weaken the contract, so selection does not narrow that stage. Mutation uses whole packages, zero missed, with fresh outcomes; cargo-mutants 27.1.0 has an executable-only cache keyed by OS, architecture, toolchain and version. No hosted macOS mutation job is added. The opt-in local hook uses the actual incoming ref endpoints, and selected mutation is an explicit `MAKNAE_PRE_PUSH_MUTANTS=1` option. Real Git history fixtures establish docs-only PR/merge behavior, reverse-dependency inclusion and conservative fallbacks; gate fixtures remain independent evidence that coverage enforcement fires.


### Correction — 2026-09-06, documentation gate preservation

The first affected-CI implementation skipped external-authority lint with the Rust build job on documentation-only changes. That gate scans `design/*.md`, `AGENTS.md` and `README.md`, so this discarded its control over exactly the inputs it checks. It now runs unconditionally in the lightweight affected job before selection. Required build/mutation/Darwin checks propagate a lightweight-job failure; a documentation lint refusal does not proceed into Rust builds. A real Git fixture introduces an invalid external citation, confirms docs-only selection, verifies unconditional workflow placement, and observes the actual lint reject the citation.

### Amendment — 2026-09-06, operational documentation and tool caches

The initial allowlist treated the existing packaging READMEs and isolation contract, deployment README, and hook README as unknown build inputs. These named documents now take the documentation-only path on both PRs and main pushes. The isolation-contract lint moves into the unconditional lightweight job beside external-authority lint, so its input can skip Rust without losing its own check. The local hook runs both documentation gates before selection. Unknown Markdown outside the existing documentation allowlist remains a build input; the rule is not a blanket file-extension exemption. Real history fixtures cover these paths, mixed code/configuration changes, and a documentation merge after main changes. The invalid-contract fixture observes the real lint refuse an empty cell and verifies placement in the lightweight workflow and local hook.

Cache the remaining gate executables with exact versions from the passing #235 merge run: cargo-deny 0.19.9, cargo-auditable 0.7.5, rust-audit-info 0.5.4, and cargo-llvm-cov 0.8.7. This resolves the tool-cache follow-on above and prevents a floating install from making cold and warm runs use different tools. Keys include OS, architecture, toolchain, and every cached tool's version; cache misses rebuild with `--locked`, and the LLVM component remains independent of the executable cache. No build outputs, test outcomes, coverage evidence, or mutation results are reused. The measured installation cost was 261 seconds per source run before cache transfer overhead; savings apply to warm caches, and hosted timings remain the evidence. Full source coverage and required job identities are unchanged.
