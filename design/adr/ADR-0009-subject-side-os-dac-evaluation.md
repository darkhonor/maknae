...
- **macOS:** `fcntl(fd, F_GETPATH)` — **written 2026-08-30, compile-checked for arm64-darwin, NEVER RUN.** `F_GETPATH` and not `F_GETPATH_NOFIRMLINK`: on APFS `/Users` is a firmlink to `/System/Volumes/Data/Users` and the two calls return the two forms; the user-visible form is what an operator writes in `principal.home` and what `realpath` reports, so it is what the confinement prefix-check compares against. If that is wrong the failure is **fail-closed** — a form mismatch makes `strip_prefix` fail, which denies — never a bypass.
...
- **MEASURED 2026-08-30 (RHEL 10.2, systemd 257, `systemd-run --user`): `d_path` is unaffected by `ProtectHome=`, in every mode.** The concern was that `d_path` is computed against the reading process's mount namespace, so a masked `/home` might render a received descriptor unreachable. It does not. An fd opened outside the sandbox and inherited into a unit resolved to `/home/aackerman/dpath-probe.txt` and read its bytes correctly under `read-only`, under `yes` (where `ls /home` is `Permission denied`), and under `tmpfs` (where `/home` is replaced outright). The reason is that `ProtectHome` **overmounts** `/home`; it does not detach the mount the descriptor was opened on, and `d_path` walks that mount.
  ...
  **A macOS CONDITION, recorded because it binds (2026-08-30).** macOS is a **deployment target**, not a dev convenience — `packaging/isolation-contract.md` said otherwise and was corrected in place after that framing was used, on this very ADR, to argue an unverified lane was acceptable. It is not. Verification runs on **Wrathion** (the operator's Apple Silicon host) and nowhere else: CI is `ubuntu-latest` only and all three standing test hosts are Linux. **Until the delegated suite has been run there, macOS support is written, not delivered**, and [#186](https://github.com/darkhonor/maknae/issues/186) does not close.
...
- **Correlation is FIFO, and that is sound — measured, not assumed.** The kernel does not merge ancillary data across `sendmsg` boundaries: with two pipelined requests each carrying a descriptor, a 4096-byte `recvmsg` returned only the first message's 8 bytes and its single fd, and the second read returned the second. Descriptors arrive in the same order as the frames they accompanied, so **taking the oldest unconsumed descriptor for each fd-bearing request is correct even under pipelining**. A client that attaches descriptors out of order only denies itself.
