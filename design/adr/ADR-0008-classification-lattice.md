...
- **Status:** Accepted (implemented in `crates/maknae-dcs-core`; PR pending operator review)
- **Date:** 2026-08-04
...
3. **`decide(subject, resource, action, purpose, spif)` is a fail-closed conjunction** of gates (policy, level-with-compilation-floor, categories, releasability, action, need-to-know). Every degenerate input — unknown level/tag/token, malformed origin, empty category value-set, Permissive tag in a label — denies (`Indeterminate`), never defaults open. **No role or administrative bypass exists.** The principal is polymorphic: human or LLM endpoint, identical treatment.
...
Assessor framing: the engine upgrades the evidence class for access-enforcement from procedural attestation to mechanized proof — the lattice laws and fail-closed totality are exhaustively machine-checked in CI, and surviving-mutant triage (spec §6.5) gates release.
...
| Verification | 576-label exhaustive law suite + golden vectors + mutation gate | SA-11; SA-11(1) | The crate's `tests/` + CI |
