...
3. **Parent-directory readability is a contract precondition of anchored I/O.** Directories holding maknae-managed artifacts must grant read (not merely search) permission to the reading principal. This is documented on `maknae_io::open_anchor` / `open_anchor_resolved` as part of the crate contract. An `O_PATH`-based lane restoring search-only compatibility is possible on Linux but is **not** built speculatively; it becomes contract work only if a real hardened-layout deployment requires it.
