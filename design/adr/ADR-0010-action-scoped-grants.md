...
> **Corrected in place 2026-08-31 — the pin has since fired, once, and this decision no longer holds for all three terms.** `admin.config.show` gained a dispatch (`Dispatch::ConfigShowRequested`) and now returns a `ConfigView`; the pin went red exactly as designed and was narrowed to `admin.status` and `admin.subject.list`, which kept it *at that time*. **Decision 15 below is what answered the question for `config.show`** — read it before relying on anything in this paragraph.
>
> **Corrected again 2026-09-01 — the pin fired a SECOND time and is now retired, so this decision holds for NO term.** Phase 3 gave `admin.status` and `admin.subject.list` their own dispatches; `the_remaining_grantable_terms_have_no_behaviour_yet` is replaced by `each_grantable_term_dispatches_to_its_own_arm`, and the Consequences bullet below records the retirement. All three grantable terms now dispatch and disclose. **Decisions 16 and 17 are what answered the question for these two** — read them before relying on anything above. The first correction narrowed this decision instead of dating its expiry, so a reader arriving one round later was told a `NoBehaviour` pin still guards two terms that in fact disclose; under the repo's authority order (code → KLC → ADRs) that is prose contradicting the code it describes.
>
...
> **Corrected again 2026-09-01.** The banner above named `docs/configuration.md` as *the* authority. It documents `core` (§4) and `lake` (§5) and nothing else — it is silent on `vault`, `transport`, `audit` and `principal`, which `boot.rs` registers today and which hold thirteen of the (then) nineteen disclosed entries *(the list has since grown to 24; the count is left as written because it dates the measurement, and re-deriving it is `config-disclosure-drift.sh`'s job, not prose's)* and the suppression the whole mechanism was built for. So the second banner sent the next author to an empty grep, exactly as the first sent them to a parser that names nothing.
>
> **The fix is not a third instruction.** `ci/gates/config-disclosure-drift.sh` enumerates the surface — every field of the declared config structs must carry a recorded decision; the code's two lists must agree with the manifest exactly in both directions; the SURFACE inventory is cross-checked against the `SectionSpec` registrations in `boot.rs`, with the number resolved asserted equal to the number registered, so a registration form the gate cannot read is a hard failure rather than a silent gap; each entry declares its field count, so a dropped field or a dropped entry changes a number someone must edit deliberately; and a field whose TYPE is a SUBTREE — a config struct, an enum, a `HashMap`/`BTreeMap`, `serde_json::Value`, or this crate's own map-bearing `Value` — must carry its own coverage, because a scalar turning into one of those otherwise changes no count at all. *(Precise scope, recorded because an over-broad claim is what this gate replaced: sections carried **verbatim** — `core.*`, `lake` — belong to no struct, so their manifest rows are not gate-derived. Adding to `DISCLOSABLE` is still caught in both directions. What is NOT gate-derived and rests on review: a new verbatim `core.*` key, and a deployer-invented key inside any section — ~~no section parser rejects unknown keys within its own block, so `vault: { my_notes: … }` loads and its key name is disclosed with the value masked~~ **(CORRECTED 2026-09-19 by #210, and NARROWED the same day when review caught this banner overstating it. What IS closed: every section with a parser refuses a key it does not read — `vault: { my_notes: … }` returns `ConfigError::UnknownKey` at boot, before any disclosure, so it cannot be disclosed — and `core`'s OWN level is closed where core is consumed, in `maknae-kernel`'s `boot.rs`. What is NOT: `core.identity` is deliberately left open (maintainer direction, #210), and nothing parses inside it, so `core: { identity: { my_notes: … } }` still loads AND still ships its key NAME through `admin.config.show` with the value masked — `SUPPRESSED` carries `core.handling` but no `core.identity` prefix, so `classify` falls through to `Mask`. That is this clause's original residual, and it survives wherever an ACCEPTED key's value is not type-forced — not in one subtree, as a first draft of this banner claimed. The rule is the durable part and the enumeration is NOT exhaustive -- a first draft of this banner enumerated three routes and review found a fourth by execution, in the very section this clause opens by calling closed. Routes measured 2026-09-19, examples not a census: `core.identity` (open by direction, nothing parses inside it); `core.schema_version` (accepted at `boot.rs`, read by nothing); `vault`'s three `get_str`-read keys (`approle_mount`, `pki_int_mount`, `deployment_id`) -- so the `vault: { my_notes: … }` example above closes only the TOP level of `vault`, and a key invented one level DEEPER still ships its name; and `audit`'s shape-tolerant keys, `jsonl_path` as well as `siem` — `audit: { siem: { my_notes: … } }` passes the vocabulary because `siem` is accepted, then `get(section,"siem").and_then(as_str)` yields `None` for a map rather than refusing (the deliberate wrong-SHAPE-defaults-one-field decision), so `flatten` still produces `siem.my_notes` and `classify` falls through to `Mask`. `lake` and `audit.au3_1` are prefix-suppressed and leak nothing. Closing the vocabularies BOUNDS this residual to accepted keys; it does not eliminate it. Also unchanged: a new verbatim key ADDED to a parser's list without a disclosure decision, which is the direction `config-disclosure-drift.sh` covers.)**)* Adding a config field without deciding its disclosure now fails CI. The completeness of this control no longer rests on prose telling someone where to look, because that prose has been wrong every time it was written. None of the disclosed fields carry secret material, because Maknae's secrets are not in `maknae.yaml` at all (they arrive via `$CREDENTIALS_DIRECTORY` and sealed files under `private/`, which this view never reads). What deny-by-default still buys is that a field added *tomorrow* masks until someone classifies it too.
>
...
**Withheld, each with its reason** (recorded next to the list in `document.rs`, because "absent from the list" and "considered and withheld" are different states and only one survives review): `audit.au3_1` — **omitted, above** (not merely masked; the first decision applied the value rule to a key problem). `audit.siem` — an offload endpoint with no schema, no validator and no consumer *(corrected 2026-09-05, #189: it HAS a consumer now — the boot gate refuses to start when it is set, since the offload it promises is unimplemented until #223. The masking decision is unchanged: the credential-in-URL hazard below is what motivates it, not the absence of a consumer)*, whose common real-world shapes embed a credential in the URL. `core.handling.*` — omitted, above. `lake` — **omitted, above**; "no parser reads it" is the premise of that conclusion, not of masking.

This is the ruling Phase 2 was blocked on. It settles `admin.config.show`. *(Corrected 2026-09-01: this said the other two terms' response shapes "remain open". They were settled in the same branch by decisions 16 and 17 below, which were appended without amending this sentence — the identical failure decision 12's banner describes, two decisions later.)*

**16. `admin.status` discloses version, protocol version, listener, and the deciding backend.** All four are deployment shape, none is a credential. The verb's own wire doc warns it is *"useful to an operator, and useful to an attacker fingerprinting the deployment"* — true, and it is why the term ships **ungranted** and admin-only. Once an operator has granted it, withholding the daemon's own version from them protects nobody: any peer that completed a handshake already knows the protocol version, and the **socket path is the one the caller is already connected to**. *(Corrected 2026-09-01, then corrected again the same day. The original said the socket path "is disclosed by `admin.config.show` on the same reasoning" — insufficient, because the grants are INDEPENDENT: a role granted only `admin.status` never gets `config.show`, so that argument cannot carry this field. The first correction went further and claimed `config.show` discloses nothing of the sort on a default deployment, since there is no `transport:` key. **That was false**, and it is the worse error of the two: `effective_view` folds the RESOLVED default in through `merge_resolved` regardless of the file, `transport.socket_path` is on `DISCLOSABLE`, and `document.rs`'s own test starts from an empty view and asserts the folded value. The correct statement is the independence point alone.)* `authz_backend` is asked of the PDP rather than hardcoded — with the classification library present the deciding backend is not `-basic`, and an operator debugging a verdict needs to know which one produced it. **`ConjunctionAuthorizer` implements it too**, joining its operands' names: taking the `unknown` default there would have made this field useless in the one deployment that motivates disclosing it. *(Scope, stated because the control is otherwise easy to over-read: those composed impls are PRE-WIRED for a build that does not exist yet. Production composes with the free function `combine(vec![guarded_decide(..)])`, which operates on `Verdict`s and has no seam at all, and ~~the kernel is handed a concrete `BasicAuthorizer`~~ the kernel is handed a `Composition<BasicAuthorizer>` *(2026-09-06)*. ~~Adopting a second backend therefore means SWITCHING to `ConjunctionAuthorizer`~~ Adopting a second backend meant a heterogeneous root that folds through the same seam free functions *(2026-09-06)* — adding an operand to `combine` alone would compose the decision while leaving `backend_name` and `subjects` reporting `-basic`, which is the exact failure this paragraph claims to prevent.)* *(Corrected 2026-09-06, #148/#154: that build exists now. Production hands the kernel a `Composition<BasicAuthorizer>` — `crates/maknae-kernel/src/composition.rs`, a HETEROGENEOUS root with the baseline and the ceiling as named fields — not a concrete `BasicAuthorizer` and not `ConjunctionAuthorizer`; the second operand was adopted by lifting `ConjunctionAuthorizer`'s bodies into seam free functions both composers call, so `backend_name` and `subjects` compose exactly as this paragraph requires: `admin.status` reports `maknae-authz-basic+maknae-ceiling`.)* Correspondingly, `subjects()` on the composed authorizer answers **only when exactly one operand can enumerate** — two answering operands are two different claims about who holds what, and this layer has no basis to pick one or merge them; under deny-overrides a merged list would not describe the composed decision either.
