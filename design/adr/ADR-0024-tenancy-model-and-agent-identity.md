...
- **Supersedes one clause of [ADR-0018](ADR-0018-client-authentication-and-authorization-gate.md)** — decision 1's *"multi-user hardening collapses into the Kubernetes/gateway case."* Its anti-friction rationale stands; see Consequences.
...
[ADR-0018](ADR-0018-client-authentication-and-authorization-gate.md)'s rule — the per-request authorization principal is the **uid** (local) or the gateway-asserted OIDC principal (remote) — applies **uniformly, with no exception for the agent runtime**.
...
**Consequence for the trail:** every record already answers *which human* an action was for, because `source.uid` and `subject.user` are the invoking human's ([ADR-0019](ADR-0019-audit-record-model.md), amended 2026-09-10). An agent's action is not a separate identity in the audit trail; it is that human's action, attributed to them.
...
- **The audit trail already satisfies this.** No record change is owed: `subject.user` and `subject.role` (2026-09-10) name the human and the role every decision was made on.
...
| Every action is attributable to a human | `source.uid` + `subject.user` + `subject.role` on every record | AU-3, AU-3(1) |
