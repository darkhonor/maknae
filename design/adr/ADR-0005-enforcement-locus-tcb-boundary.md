...
> **Amendment (2026-08-22, via [ADR-0006](ADR-0006-client-authentication-model.md) — Accepted).** Decision 2's rule that the daemon serves a client only on a verified `cli`-plane leaf gains a **bootstrap exception**: leaf *issuance* cannot itself require the leaf it issues. On the issuance handshake the channel is still TLS 1.3 with the daemon server-authenticated (`plane/kernel` cert), but the *client* is authenticated by a leaf-independent factor — on the **local, direct-to-daemon plane this ADR governs**, the `SO_PEERCRED` uid (∈ `maknae` group) — after which the trust plane brokers and returns the short-lived `cli` leaf and every subsequent request uses full mutual mTLS as decided here. **The *remote* path is different and out of scope here** — per ADR-0006 Decision 6 a remote client reaches a **gateway**, not `maknaed`; the gateway presents its **own** server identity (never `plane/kernel`) and the remote client holds **no `cli` leaf** at all (its session is `client↔gateway`) — so this banner's concrete `plane/kernel`/`cli`-leaf mechanics describe the local plane only. This **refines, not weakens, "mTLS from the start"** (the channel is never plaintext; only the client-auth factor for the one issuance handshake differs). See ADR-0006 for the client-authentication model.
