...
> **Amendment (2026-08-22, via [ADR-0006](ADR-0006-client-authentication-model.md) — Accepted).** The **client-channel half of the mutual-mTLS requirement is superseded in target model**: X.509 plane leaves become **machine identity only** (the daemon today; the gateway's internal channel later) — humans and client apps never hold a Maknae certificate or any Vault credential. A **local client** authenticates by the kernel-attested `SO_PEERCRED` uid (∈ root-gated `maknae` group) over the `0660` UDS, with the daemon server-authenticated (`plane/kernel` cert, TLS 1.3) and identity riding a **short-TTL session bound to that uid** (live peer-cred re-verified against the session on every connection). A **remote client** terminates at the **gateway** (#66) — its own machine identity, never the daemon's `plane/kernel` key — authenticates by enrolled-subject OIDC, and never reaches `maknaed`; the `gateway↔daemon` channel remains **mutual mTLS (machine-to-machine)**. **No new Vault signing capability is granted to anyone** — the per-plane policy scoping and custody-by-secret-absence below are unchanged; the `cli` PKI role is **retired as a client credential** (its disposition — gateway machine identity or removal — is decided with #66). **Prospective, not yet in force:** until the session path lands, the shipped model (CLI SecretID + self-minted `cli` leaf + mutual mTLS) remains in effect (#73 OPEN). The channel is never plaintext at any point in either model. See ADR-0006 for the trust assumptions, threat model, and session contract.
