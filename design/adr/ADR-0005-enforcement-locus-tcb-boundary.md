...
> **Amendment (2026-08-22, via [ADR-0006](ADR-0006-client-authentication-model.md) — Accepted).** The **client-channel half of the mutual-mTLS requirement is superseded in target model**: X.509 plane leaves become **machine identity only** (the daemon today; the gateway's internal channel later) — humans and client apps never hold a Maknae certificate or any Vault credential. A **local client** authenticates by the kernel-attested `SO_PEERCRED` uid over the `0660` UDS, with the daemon server-authenticated (`plane/kernel` cert, TLS 1.3); on **every connection — and re-evaluated on every request over that connection** — the daemon verifies the live peer-cred uid (kernel-fixed at connect), *current* `maknae`-group membership (re-resolved from the host user database, never the client's snapshotted groups), **and a live host-login session for that uid** (host session registry / logind) — so de-enrollment **and logout** terminate access at the next **request**, even over an already-open connection; lookup or registry failure fails closed, audited as an infrastructure denial; identity is held as a **daemon-side session record keyed to (uid, connection)** — **no token or handle is ever issued to the client, and nothing is persisted client-side** (#115). A **remote client** terminates at the **gateway** (#66) — its own machine identity, never the daemon's `plane/kernel` key — authenticates by enrolled-subject OIDC, and never reaches `maknaed`; the `gateway↔daemon` channel remains **mutual mTLS (machine-to-machine)**. **No new Vault or PKI capability is granted to anyone** — machine-plane policy scoping and custody-by-secret-absence are unchanged, and retiring the `cli` client role strictly *narrows* the grant surface (the role's disposition — gateway machine identity or removal — is decided with #66). Four further clauses of this ADR are superseded **for the client channel only**: the **mTLS negative suite's "absent client cert → reject" criterion** (retained for machine channels — in the target model `maknaed` runs distinct verifier configurations per listener, and acceptance tests assert each channel class separately, #114); the negative suite's **"uid mismatch" rejection** (with the client cert retired there is no second identity source to mismatch — its replacement negatives are the group-gate rejections, a non-member refused at connect and a de-enrolled member refused at the next request, both constructible and fail-closed); the "`maknaed` requires `plane/cli`" peer-verifier rule (machine channels keep URI-SAN verification); and the `maknae-vault` crate-contract's client-side leaf-minting path (#116). When #66 lands, the **gateway joins the trust plane as the remote PEP and principal-asserter** — a named TCB expansion (ADR-0006 trust assumption 5), not an implicit one. **Prospective, not yet in force:** until the session path lands (#114/#115/#116), the shipped model (CLI SecretID + self-minted `cli` leaf + mutual mTLS) remains in effect (#73 OPEN). The channel is never plaintext at any point in either model. See ADR-0006 for the trust assumptions, threat model, and session contract.
