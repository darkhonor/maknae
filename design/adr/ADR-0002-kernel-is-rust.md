...
| Cryptography | `aws-lc-rs` — FIPS 140-3 validated module (CMVP #5314 static / #5298 dynamic, the AWS-LC FIPS 3.x line pinned in `maknae-vault`), proven in-house by Microkosmos. *Corrected 2026-09-11 (#200): the certificate's tested operational environments are Amazon Linux 2023 on Graviton4 and Xeon 8375C only; every platform Maknae ships today — RHEL/Rocky 9 and macOS on Apple Silicon — runs the validated module as a ported, unlisted environment. The claim is "validated module", never "validated on this platform"; `packaging/isolation-contract.md` carries the ruling.* | SC-13 (cryptographic protection — FIPS-validated mechanisms) | CMVP validation record for the AWS-LC module (#5314 security policy, Table 3) |
