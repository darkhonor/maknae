...
| Cryptography | `aws-lc-rs` — the AWS-LC FIPS module line, pinned in `maknae-vault`, proven in-house by Microkosmos. *Corrected 2026-09-11 (#200, PR #284), re-corrected 2026-09-13: this row said "FIPS 140-3 validated module". CMVP #5314 (static) / #5298 (dynamic) validate this line on **Amazon Linux 2023 with Graviton4 or Xeon 8375C only**. SC-13 evidence is therefore incomplete on **ONE** axis — the **operational environment** — on every platform Maknae ships today (RHEL/Rocky 9, macOS on Apple Silicon), which run the validated module line outside its tested environments. The claim permitted is "validated module line", never "validated on this platform". **MAINTAINER RULING 2026-09-13 (restated; first given 2026-09-12): ANY 3.x implementation is approved.** The AWS-LC-FIPS **3.x line** is what CMVP validated and what an assessor evaluates; that the certificate was issued at 3.1.0 while the pinned crate vendors 3.6.0 is **not a finding** and is not tracked. Prose that treated it as an unresolved 'version mapping' barring citation of the certificate is struck and must not be reintroduced. `packaging/isolation-contract.md` carries both rulings.* *Corrected 2026-09-12 (#227): this row cites both certificates without saying which applies where, and #5314 (static) **cannot** apply on macOS — upstream refuses a static FIPS build outside Linux (`aws-lc/CMakeLists.txt:842`) and calls shared *"the required form for FIPS on macOS and Windows"* (`aws-lc-fips-sys/README.md:143`). **Linux maps to #5314 (static); macOS maps to #5298 (dynamic).** Neither licenses a "validated on this platform" claim, because neither certificate lists an environment Maknae ships on — that is the environment axis above, and it is the only one. A document naming #5314 in a macOS context names the wrong module entirely.* | SC-13 (cryptographic protection — FIPS-validated mechanisms) — evidence incomplete, see the correction | CMVP #5314 security policy Table 3 (environments) — Linux; #5298 — macOS |
