...
| Cryptography | `aws-lc-rs` — the AWS-LC FIPS module line, pinned in `maknae-vault`, proven in-house by Microkosmos. *Corrected 2026-09-11 (#200, PR #284): this row said "FIPS 140-3 validated module". CMVP #5314 (static) / #5298 (dynamic) validate this line at module version **3.1.0**, on Amazon Linux 2023 with Graviton4 or Xeon 8375C only; the crate Maknae pins builds **3.6.0**, and whether that build is inside the certificate's scope is unresolved (#200). SC-13 evidence is therefore INCOMPLETE on two axes — version mapping and operational environment — on every platform Maknae ships today (RHEL/Rocky 9, macOS on Apple Silicon). The claim permitted today is "built on the AWS-LC FIPS module line" — not "validated module", and never "validated on this platform". `packaging/isolation-contract.md` carries the ruling.* *Corrected 2026-09-12 (#227): this row cites both certificates without saying which applies where, and #5314 (static) **cannot** apply on macOS — upstream refuses a static FIPS build outside Linux (`aws-lc/CMakeLists.txt:842`) and calls shared *"the required form for FIPS on macOS and Windows"* (`aws-lc-fips-sys/README.md:143`). **Linux maps to #5314 (static); macOS maps to #5298 (dynamic).** Both remain subject to the same unresolved #200 version mapping, so neither licenses a "validated module" claim — but a document naming #5314 in a macOS context names the wrong module entirely.* | SC-13 (cryptographic protection — FIPS-validated mechanisms) — evidence incomplete, see the correction | CMVP #5314 security policy Table 3 (environments); the version-to-certificate mapping owed by #200 |
