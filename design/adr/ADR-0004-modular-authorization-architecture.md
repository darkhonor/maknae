...
`maknae-security` is a **semver-versioned crate**, and the contract it publishes — the `Authorizer` trait plus the `Request` / `Verdict` / `Obligation` / `AttrValue` types — is its public API under that semver. A breaking change to the contract is a **major version bump** for third parties who **pin a contract version**, and the narrow waist exists precisely to keep major bumps rare. The **§2** attribute-value enum (`AttrValue`) is the surface most likely to move — it is a revisable starting point; a change there is at minimum a minor bump and triggers a re-check of the obligation model, whose params ride on that same enum (also §2). The intent is explicit: **stop the goalposts moving** — an external backend author builds against a pinned, stable contract version, not a shifting surface.

> **Updated 2026-08-31.** This paragraph previously held the contract "to the same discipline as the `maknaed`↔`maknae` protocol (AGENTS.md, *protocol/daemon-contract discipline*)". **That section no longer exists** — by operator ruling nothing is a breaking change while Maknae is still building, and the operator is sole governance on protocol and wire semantics. What survives here is the semver promise to a third party who has pinned a version, which is a different thing and does not depend on the deleted text.
