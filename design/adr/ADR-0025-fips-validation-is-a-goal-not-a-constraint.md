...
`crates/maknae-vault/Cargo.toml` requires `aws-lc-rs = "1.18"` (floor, **no ceiling**), which resolves `aws-lc-fips-sys` 0.14.x — the 4.x module. The mirrored `rcgen` ceiling and both `.github/dependabot.yml` ignore rules are removed with it. `aws-lc-rs` stays a **named** dependency of `maknae-vault` so the workspace resolves one version and the posture has one documented home.
...
**The dylib basename moves with the crate version** on macOS: `libaws_lc_fips_0_13_17_crypto.dylib` → `libaws_lc_fips_0_14_2_crypto.dylib`. *(The rule is measured, the `.dylib` name is derived from it: this host is Linux, where both artifacts are present as `libaws_lc_fips_0_13_17_crypto.a` and `libaws_lc_fips_0_14_2_crypto.a` under `target/debug/build/aws-lc-fips-sys-*/out/build/artifacts/`. The macOS dynamic form has not been observed on a Mac in this change; CI's `darwin-native` lane and the `.pkg` smoke test are where it is.)* `packaging/macos/build-pkg.sh` globs it and `smoke.sh` normalises it before comparing the bill of materials, so packaging needed no change — only the README's quoted measurement did.

**Documents corrected in place, dated, per the in-artifact rule — the complete set, because a later sweep will be driven from this list:** `crates/maknae-vault/Cargo.toml` (both comment blocks), `.github/dependabot.yml`, `crates/maknae-vault/src/fips.rs`, `crates/maknae-llm/Cargo.toml`, `packaging/isolation-contract.md`, `packaging/macos/README.md`, ADR-0002's SC-13 row, ADR-0009's FIPS parenthetical, the `design/references/2026-08-30-fips-supply-chain-visibility.md` record (whose dependency-visibility findings are untouched by this — they never turned on certificate status), **`AGENTS.md`** (a standing ruling, so the pin is not proposed again), **`design/adr/README.md`** (this registry row) and **`ci/gates/darwin-cross-check.sh`** (a dated version note on a quoted measurement). Each of them links back here: this ADR is the decision of record, and the manifest comment mirrors it rather than replacing it.
