...
**Sites corrected in place, dated, per the in-artifact rule — a NON-EXHAUSTIVE list, and deliberately so.** The authoritative set is `git log main..` for this change; what follows is a reader's map, not an inventory, and nothing here asserts it is complete.

*Why it is written that way.* Six review rounds ran against this change, and **this one sentence was the finding in five of them** — each time it claimed to name every corrected site, and each time the sweep behind it had missed one. The misses migrated: first the standards register, then `deny.toml`, then `CONTRIBUTING.md`, then `crates/maknae-vault/src/fips_glue.rs` and `docs/runbook.md`, then `crates/maknae-llm/tests/stub_server.rs` and `crates/maknae-vault/src/digest.rs`. The last four were in `crates/` — the tier [AGENTS.md](../../AGENTS.md)'s authority order ranks **above** every document listed here — and two of them hid from a line-oriented `grep` because the source wrapped the word to the start of a line. **A completeness claim is a claim**, and this one kept being false in the decision record a reader is sent to first. The rule that follows, for this file and any other: do not assert a set is complete unless something mechanically derives it. Where a human writes the list, say it is partial.

The map, then. **Manifests and gates:** `crates/maknae-vault/Cargo.toml` (both comment blocks), `.github/dependabot.yml`, `deny.toml`, `.github/workflows/ci.yml`, `ci/gates/darwin-cross-check.sh`. **Source, which outranks all the prose below it:** `crates/maknae-vault/src/fips.rs`, `crates/maknae-vault/src/fips_glue.rs`, `crates/maknae-vault/src/client.rs`, `crates/maknae-vault/src/digest.rs`, `crates/maknae-llm/Cargo.toml`, `crates/maknae-llm/tests/stub_server.rs` — the last five because each gave a struck rationale (a `ring` fallback, or a "pin" that is no longer the control) for something still true; **every ordering and fail-closed requirement they describe is unchanged.** **Claim registers and docs:** `design/diagrams/standards-profile.toml` and its rendered SVG, `packaging/isolation-contract.md`, `packaging/macos/README.md`, `packaging/macos/build-pkg.sh`, `packaging/macos/smoke.sh`, `packaging/common/maknaed.service`, `docs/runbook.md`, ADR-0002's SC-13 row, ADR-0009's FIPS parenthetical, `design/adr/README.md`, `design/references/2026-08-30-fips-supply-chain-visibility.md`, `design/references/2026-08-21-agent-deck-assessment.md`, `CONTRIBUTING.md`, and `AGENTS.md` (a standing ruling, so the pin is not proposed again). `Cargo.lock`, `THIRD-PARTY-NOTICES.md` and the stamp-only SVGs are covered elsewhere in this ADR.

**This sentence has been the finding in four consecutive review rounds**, each time naming a set smaller than the change actually corrects, and it is left on the record rather than quietly fixed because the pattern is the lesson: a completeness claim is a claim, and this file is where AGENTS.md's authority order sends a reader. The last two omissions were in `crates/` — the tier that outranks every document listed here — because the sweep that found them grepped for a space-delimited word and the source had wrapped it to the start of a line.

**The standards register is the one that mattered most, and the one the first pass missed.** `design/diagrams/README.md` names `standards-profile.toml` as where standards claims live — *"curated, reviewable, every row citing evidence"*, explicitly in preference to *"prose scattered across ADRs"*. Its FIPS 140-3 row claimed `version = "aws-lc-fips (BoringCrypto module)"` with no CMVP qualifier (and named the wrong module: AWS-LC-FIPS is not BoringCrypto). Correcting the prose sites while leaving the designated register asserting the struck claim would have left the contradiction in the one artifact built to be authoritative about it. The row now reads `AWS-LC-FIPS 4.x — in CMVP review` and cites this ADR. **`status` stays `enforced` deliberately**, per that file's own definitions: what is mechanically enforced is that the process runs the FIPS provider or refuses to start. The certificate state is not a `status` value and now lives in `version`, where the rendered StdV-1 shows it. Each of them links back here: this ADR is the decision of record, and the manifest comment mirrors it rather than replacing it.
