...
> **Authority.** Authorization, enforcement locus, audit, and the access-control model are governed by the ADRs, not this doc: [ADR-0004](adr/ADR-0004-modular-authorization-architecture.md) (the modular `maknae-security` seam + pluggable `maknae-authz-*` backends), [ADR-0005](adr/ADR-0005-enforcement-locus-tcb-boundary.md) (split-kernel, mTLS, sole PDP), [ADR-0018](adr/ADR-0018-local-plane-authorization-deployment-model.md) (local-plane authz + enrollment), [ADR-0019](adr/ADR-0019-audit-record-model.md) (audit record model), [ADR-0020](adr/ADR-0020-access-control-model-and-vocabulary.md) (RBAC/ABAC over DAC/MAC). The DCS classification engine is external (an optional external library), reached only through the seam *(clarified 2026-09-06, #148/#154: the SCALPEL is external; a level CEILING over the four-level order is in-repo, a non-removable operand of the kernel's `Composition`)*. Where this doc and an ADR disagree, the ADR wins.
