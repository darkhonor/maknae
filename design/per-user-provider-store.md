...
3. **The deputy deliberately reads almost nothing.** Its only configuration input is `/etc/maknae/egress-bounds.yaml` — two keys, `kv_mount` and `key_vault_path_prefix`. The stated invariant (`crates/maknae-config/src/bounds.rs`): *"egress reads no policy and no registry; it may read its own operating bounds."*
