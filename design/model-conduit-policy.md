...
- **The deputy performs its own resolution** against the declared set — never `getaddrinfo`, never `/etc/hosts`, never multicast. A pure-Rust DNS client keeps [ADR-0002](adr/ADR-0002-kernel-is-rust.md) intact; DNS-over-TLS or DNS-over-HTTPS to the declared resolver runs over the FIPS provider, so the resolver's *identity* is TLS-verified and the query is confidential.
...
**DNSSEC, per source rather than globally.** A validating resolver gives cryptographic integrity of the *answer*, independent of the resolver's honesty. Requiring it everywhere would refuse much of the vendor tier — large code-hosting domains are unsigned. But `.mil` and `.gov` zones are signed by mandate (OMB M-08-23), and those are precisely the authority-map sources. `dnssec: required` is therefore plausibly a per-source attribute in the lake's registry: the authority tier gets the stronger guarantee, the vendor tier gets TLS alone. Deferred — a registry attribute, not a mechanism, and cheap when wanted.
...
- **A declared resolver is named, not verified.** Declaring the resolver makes it a policy input on the audit record; it does not make its answers correct. Answer integrity is DNSSEC's job where the zone is signed and TLS's job everywhere; the address policy bounds reach regardless.
